...
namespace OctolizeShippingAustraliaPostVendor\GuzzleHttp\Handler;

use OctolizeShippingAustraliaPostVendor\GuzzleHttp\Exception\ConnectException;
use OctolizeShippingAustraliaPostVendor\GuzzleHttp\Exception\RequestException;
use OctolizeShippingAustraliaPostVendor\GuzzleHttp\Promise\FulfilledPromise;
use OctolizeShippingAustraliaPostVendor\GuzzleHttp\Psr7;
use OctolizeShippingAustraliaPostVendor\GuzzleHttp\Psr7\LazyOpenStream;
use OctolizeShippingAustraliaPostVendor\GuzzleHttp\TransferStats;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx)
    {
        ...
        if ($easy->onHeadersException) {
            return \OctolizeShippingAustraliaPostVendor\GuzzleHttp\Promise\rejection_for(new RequestException('An error was encountered during the on_headers event', $easy->request, $easy->response, $easy->onHeadersException, $ctx));
        }
        ...
        return \OctolizeShippingAustraliaPostVendor\GuzzleHttp\Promise\rejection_for($error);
    }
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf)
    {
        ...
        if (isset($options['sink'])) {
            $sink = $options['sink'];
            if (!is_string($sink)) {
                $sink = \OctolizeShippingAustraliaPostVendor\GuzzleHttp\Psr7\stream_for($sink);
        ...
        if (isset($options['proxy'])) {
            if (!is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) || !\OctolizeShippingAustraliaPostVendor\GuzzleHttp\is_host_in_noproxy($host, $options['proxy']['no'])) {
        ...
        if (!empty($options['debug'])) {
            $conf[\CURLOPT_STDERR] = \OctolizeShippingAustraliaPostVendor\GuzzleHttp\debug_resource($options['debug']);
