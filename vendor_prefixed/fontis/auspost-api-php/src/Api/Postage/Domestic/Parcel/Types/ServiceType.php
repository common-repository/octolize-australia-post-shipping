...
namespace OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Api\Postage\Domestic\Parcel\Types;
...
use OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Model\Postage\PostageType;
...
class ServiceType
{
    ...
    /**
     * @return PostageTypeCollection $typeCollection
     */
    public function listTypes(): PostageTypeCollection
    {
        $response = $this->httpClient->get('postage/parcel/domestic/type.json');
        $responseBody = $this->parseResponse($response);
        if (!isset($responseBody["types"]) || !isset($responseBody["types"]["type"]) || !count($responseBody["types"]["type"])) {
            return new PostageTypeCollection([]);
        }
        $result = [];
        foreach ($responseBody["types"]["type"] as $type) {
            $code = $type["code"] ?? "";
            $name = $type["name"] ?? "";
            array_push($result, new PostageType($code, $name));
        }
        $typeCollection = new PostageTypeCollection($result);
