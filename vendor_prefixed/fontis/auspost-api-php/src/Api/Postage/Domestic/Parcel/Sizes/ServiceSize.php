...
namespace OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Api\Postage\Domestic\Parcel\Sizes;
...
use OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Model\Postage\PostageSize;
...
class ServiceSize
{
    ...
    {
        $response = $this->httpClient->get('postage/parcel/domestic/size.json');
        $responseBody = $this->parseResponse($response);
        if (!isset($responseBody["sizes"]) || !isset($responseBody["sizes"]["size"]) || !count($responseBody["sizes"]["size"])) {
