...
namespace OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Api\Postage\International\Country;
...
use OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Model\Postage\Country;
...
class ServiceCountry
{
     ...
     */
    public function listCountries(): CountryCollection
    {
        $response = $this->httpClient->get('postage/country.json');
        $responseBody = $this->parseResponse($response);
        if (!isset($responseBody["countries"]) || !isset($responseBody["countries"]["country"]) || !count($responseBody["countries"]["country"])) {
            return new CountryCollection([]);
        }
        $countries = [];
        foreach ($responseBody["countries"]["country"] as $country) {
            array_push($countries, new Country($country["name"], $country["code"]));
        }
