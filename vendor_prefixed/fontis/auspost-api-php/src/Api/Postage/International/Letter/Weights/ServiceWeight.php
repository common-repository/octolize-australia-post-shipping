...
namespace OctolizeShippingAustraliaPostVendor\Fontis\Auspost\Api\Postage\International\Letter\Weights;
...
class ServiceWeight
{
    ...
    {
        $response = $this->httpClient->get('postage/letter/international/weight.json');
        ...
        foreach ($responseBody["weights"]["weight"] as $weight) {
            ...
            $weightValue = isset($weight["value"]) ? (float) $weight["value"] : null;
            $weightCode = $weight["code"] ?? "";
            array_push($result, new Weight($weightName, $weightValue, $weightCode));
        }
