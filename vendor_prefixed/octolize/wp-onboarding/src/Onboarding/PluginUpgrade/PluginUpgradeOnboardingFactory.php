...
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\Field\Html;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\Onboarding;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingButton;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingOption;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingShouldShowAlwaysStrategy;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingShouldShowStrategy;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingStep;
use OctolizeShippingAustraliaPostVendor\Octolize\Onboarding\OnboardingTrackerData;
use OctolizeShippingAustraliaPostVendor\WPDesk\Forms\Field;
/**
...
class PluginUpgradeOnboardingFactory
{
    ...
    private function prepare_steps(string $previous_version, string $current_version): array
    {
        ...
        $onboarding_step->set_show(\true)->set_heading(sprintf(__('Thank you for updating %1$s!', 'octolize-australia-post-shipping'), $this->plugin_name))->set_sub_heading(__('It is really important to keep the plugins up to date. We have implemented some improvements and new functionalities. Find out what has changed:', 'octolize-australia-post-shipping'));
    ...
    {
        return [(new OnboardingButton())->set_label(__('I\'m not interested', 'octolize-australia-post-shipping'))->set_classes(OnboardingButton::BTN_LINK), (new OnboardingButton())->set_label(__('Thanks for letting me know', 'octolize-australia-post-shipping'))->set_type(OnboardingButton::TYPE_CLOSE)];
    }
