...
namespace OctolizeShippingAustraliaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Standard;
...
use OctolizeShippingAustraliaPostVendor\WPDesk\Tracker\Shop;
    ...
    public function handle_rates(\WC_Shipping_Method $method, ErrorLogCatcher $logger, WooCommerceShippingMetaDataBuilder $metadata_builder, WooCommerceShippingBuilder $shipment_builder)
    {
        try {
            ...
            if ($rates_count === count($method->rates)) {
                $logger->info(__('No rates added from standard service rates!', 'octolize-australia-post-shipping'));
            }
        } catch (\Exception $e) {
