...
use OctolizeShippingAustraliaPostVendor\WPDesk\RepositoryRating\DisplayStrategy\DisplayDecision;
/**
...
class TextPetitionDisplayer implements Hookable
{
    ...
    public function enqueue_css_if_should()
    {
        if ($this->display_decision->should_display()) {
            wp_enqueue_style('octolize-australia-post-shipping', plugin_dir_url(__DIR__ . '/../assets/css/style.css') . 'style.css', array(), self::SCRIPTS_VERSION);
        }
    }
    /**
     * @internal
